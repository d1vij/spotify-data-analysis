...
async def fetch_track(session, track_name: str, artist_name: str, api_key: str = API_KEY, retries: int = 3) -> dict | None:
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            print(f"[ERROR] {track_name} - {artist_name} (attempt {attempt}/{retries}): {e}")
            await asyncio.sleep(2**attempt)  # exponential backoff

    return None


async def get_track_metadata_async(df: pd.DataFrame, api_key: str = API_KEY, concurrency: int = 10) -> pd.DataFrame:
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for row in df[["master_metadata_track_name", "master_metadata_album_artist_name"]].itertuples(index=False):
        ...
        print(f"[START] Fetching {len(tasks)} unique tracks with concurrency={concurrency}...")

        results = await asyncio.gather(*tasks)

    elapsed = time.time() - start_time
    print(f"[DONE] Retrieved {len([r for r in results if r])} tracks in {elapsed:.2f} seconds")
